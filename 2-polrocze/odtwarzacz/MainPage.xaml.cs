...
namespace odtwarzacz
{
    public partial class MainPage : ContentPage
    {
        ...
        private void WyswietlAlbum(int index)
        {
           if (albumy.Count > 0)
            {
                var album = albumy[index];
                lblArtysta.Text = $"Artysta: {album.artist}";
                lblTytul.Text = $"Album: {album.album}";
                lblNumeryCount.Text = $"Liczba utworów: {album.songsNumber}";
                lblRok.Text = $"Rok wydania: {album.year}";
                lblPobrania.Text = $"Liczba pobrań: {album.downloadNumber}";
            }
        }
